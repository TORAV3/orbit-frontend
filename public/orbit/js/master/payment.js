function loadData() {
  axios
    .get("http://localhost:3000/orbit/api/master/typepayment/data")
    .then((res) => {
      if (res.data.data && res.data.data.length > 0) {
        const formattedData = res.data.data.map((item) => {
          return {
            code: item.pyid, // Map API field to "code"
            desc: item.pydesc, // Map API field to "name"
          };
        });
  ...
  webix.confirm({
    ...
    callback: function (result) {
      if (result) {
        axios
          .delete(
            `http://localhost:3000/orbit/api/master/typepayment/delete/${selectedData.code}`
          )
          .then(function () {
            webix.message({
              type: "success",
...
function saveData() {
  ...
  var requestData = {
    pyid: formValues.code, // Map "code" field
    pydesc: formValues.desc, // Map "name" field
  };
  ...
  if (formValues.id) {
    ...
    axios
      .put(
        `http://localhost:3000/orbit/api/master/typepayment/edit/${formValues.code}`,
        requestData
      )
    ...
    axios
      .post(
        "http://localhost:3000/orbit/api/master/typepayment/tambah",
        requestData
      )
      .then(function (response) {
